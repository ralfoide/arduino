...
/* RM 2015-07-20 */
bool DigiFi::isRecentActivity() {
    return millis() < digiFiActivityTimeout;
}

uint8_t DigiFi::connected(){

    uint8_t ret = DIGIFI_CONNECTED_NOT;

    
    if(Serial1.available() > 0)
            return DIGIFI_CONNECTED_BECAUSE_DATA;
    
    if(millis() < digiFiActivityTimeout)
        return DIGIFI_CONNECTED_BECAUSE_RECENT;
